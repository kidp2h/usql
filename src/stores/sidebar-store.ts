...
type SidebarState = {
  ...
  addQueryPathToConnection: (connectionId: Connection['id'], filePath: string) => void;
}
export const useSidebarStore = create<SidebarState>()(
  persist(
    (set) => ({
      ...
      addQueryPathToConnection: (connectionId: Connection['id'], filePath: string) => set((state) => ({
        connections: state.connections.map((c) => {
          if (c.id !== connectionId) return c;
          const paths = c.queryPaths || [];
          if (paths.includes(filePath)) return c;
          return { ...c, queryPaths: [...paths, filePath] };
        }),
      })),
    }),
    {
